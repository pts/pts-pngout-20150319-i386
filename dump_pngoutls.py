...
def main(argv):
  parts = (
      ...
      Part('P.bss',               'NOBITS',      0x805d1f0, 0x151f0, 0x181f850),
  )
  labels = (
      ('log@plt', 0x8048a50),
      ('read@plt', 0x8048a60),
  ...
  f = open('pngoutls.golden', 'rb')
  fo = open('pngoutls.nasm', 'wb')

  addr = 0x8048000
  fo.write('; Generated by %s\n'  % argv[0])
  fo.write('; Compile: tools/nasm-0.98.39 -O0 -w+orphan-labels -f bin -o pngoutls pngoutls.nasm && chmod +x pngoutls && cmp pngoutls.golden pngoutls && echo OK\n')
  fo.write('bits 32\ncpu 386\norg 0\nR.code equ $-0x%x\nB.code equ -0x%x\nA.code equ 0\n' % (addr, addr))
  ofs = 0
  for part in parts:
    if ofs != part.off:
