...
def main(argv):
  ...
  fo = open('pngoutss.nasm', 'wb')

  addr = 0x8048000
  fo.write('; Generated by %s\n'  % argv[0])
  fo.write('; Compile: tools/nasm-0.98.39 -O0 -w+orphan-labels -f bin -o pngoutss pngoutss.nasm && chmod +x pngoutss && cmp pngoutss.golden pngoutss && echo OK\n')
  fo.write('bits 32\ncpu 386\norg 0\nR.code equ $-0x%x\nB.code equ -0x%x\nA.code equ 0\n' % (addr, addr))
  ofs = 0
  for part in parts:
    if ofs != part.off:
